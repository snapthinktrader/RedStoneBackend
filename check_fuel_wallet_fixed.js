...
async function checkFuelWallet() {
  try {
    ...
    // Get TRX balance
    const trxBalance = await tronWeb.trx.getBalance(FUEL_WALLET);
    ...
    try {
      const response = await fetch(`https://apilist.tronscan.org/api/account?address=${FUEL_WALLET}`);
      if (response.ok) {
    ...
    const account = await tronWeb.trx.getAccount(FUEL_WALLET);
    ...
    try {
      const response = await fetch(`https://apilist.tronscan.org/api/transaction?sort=-timestamp&count=true&limit=5&start=0&address=${FUEL_WALLET}`);
      if (response.ok) {
        const data = await response.json();
    ...
    console.log('✅ Fuel wallet check complete');
