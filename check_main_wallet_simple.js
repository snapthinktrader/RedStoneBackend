...
async function checkMainWallet() {
  try {
    ...
    // Get USDT balance using direct contract call
    try {
      const parameter = [{type:'address',value:MAIN_WALLET}];
      ...
      if (transaction && transaction.constant_result && transaction.constant_result[0]) {
        ...
        const usdtAmount = tronWeb.toBigNumber(balance).div(1000000).toString();
      ...
      console.log('💵 USDT Balance: 0 USDT');
      console.log('⚠️  Note: Unable to fetch USDT balance -', err.message);
    }
